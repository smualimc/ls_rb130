...
=end
  ...
  # category names. Can we do that without extracting items directly from the array based on 
  # their indices?
  ...
  # The method should yield the contents of the array to a block, which should assign your block 
  # variables in such a way that it ignores the first two elements, and groups all remaining 
  # elements as a raptors array.
  
  # Hint
  # Ruby treats block variables in much the same way that it does during assignment from an 
  # array. In fact, you can use the splat operator to collect items into an Array.

  def passing_two(ary)
    yield(ary)
  end

  passing_two(birds) {|_,_, *raptors| puts "raptors = #{raptors.join(", ")}"}